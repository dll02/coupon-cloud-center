...
public abstract class AbstractRuleProcessor {
    ...
    protected double goodsCostSum(List<Product> products) {
        return products.stream().mapToDouble(g -> g.getPrice() * g.getCount()).sum();
    }
    ...
    protected Map<Long, Double> sumProductCostIntoGroup(List<Product> products) {
        Map<Long, Double> groups = products.stream().collect(Collectors.groupingBy(m -> m.getShopId(), Collectors.summingDouble(p -> p.getPrice() * p.getCount())));
        return groups;
    }
